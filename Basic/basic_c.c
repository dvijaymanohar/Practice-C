...
int main()
{
  ...
  printf("%d\n", OFFSETOF(PodType, c));
  const static double MACSECAPP_VER = 1.0;
  //getchar();
  char str[10];
  str[0] = '\0';
  unsigned int port_num = 14;
  snprintf((char *)str, 10, "port%u", port_num);
  puts(str);
#if 0
  snprintf((char *)str, 6, "port %f", 14.55);
  puts(str);
  char buf[4];
  gcvt(MACSECAPP_VER, 4, buf);
  printf("\nMACsec Application Version %s0 Initial\n", buf);
  printf("Copyright (C) 2019 Inside Secure B.V.\n\n");
#endif
  return 0;
}
#endif

#if 0/* Log table using 0xe5 (229) as the generator */
...
int main(int argc, const char **argv)
{
  printf("%s\n", STRINGIZE(STRCAT3(PKT, 0, _RC_CLOCK_RATE)));
  return 0;
}
